...
import { ok, err, fail } from "./errors.ts";
...
class Parser {
  ...
  parse(depth: number): Result<Node> {
    return this.or(depth);
  }
  private or(d: number): Result<Node> {
  ...
  private and(d: number): Result<Node> {
    if (d > MAX_DEPTH) return fail("GuardParseError", "expression too deep");
  ...
  private cmp(d: number): Result<Node> {
    if (d > MAX_DEPTH) return fail("GuardParseError", "expression too deep");
  ...
  private unary(d: number): Result<Node> {
    if (d > MAX_DEPTH) return fail("GuardParseError", "expression too deep");
  ...
  private primary(d: number): Result<Node> {
    const t = this.eat();
...
export function compileGuard(expr: string): Result<CompiledGuard> {
...
function lookup(ctx: unknown, segs: string[]): unknown {
  let cur: unknown = ctx;
  for (const s of segs) {
...
export function evalGuard(g: CompiledGuard, ctx: unknown): boolean {
