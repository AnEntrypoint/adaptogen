// History query over the event log. The log is the audit trail: transitions,
// blocks, soft violations, rewards, evolutions. reward() credit-assignment and
// the CLI both read through here. ASCII-only, human/agent readable.

import type { DState } from "./engine.ts";
import type { DEventType, NodeId, EdgeId } from "./types.ts";

export interface HistoryFilter {
  node?: NodeId;
  edge?: EdgeId;
  type?: DEventType;
  fromSeq?: number;
  limit?: number;
}

export interface HistoryEntry {
  seq: number;
  ts: number;
  type: DEventType;
  summary: string;
  payload: Record<string, unknown>;
}

export function history(ds: DState, filter: HistoryFilter = {}): HistoryEntry[] {
  const evs = ds.store.readEvents({
    ...(filter.fromSeq != null ? { fromSeq: filter.fromSeq } : {}),
    ...(filter.type ? { type: filter.type } : {}),
  });
  const limit = filter.limit ?? 100;
  const matched = evs.filter((e) => {
    const p = e.payload as Record<string, unknown>;
    if (filter.node && p.to !== filter.node && p.from !== filter.node && p.id !== filter.node) return false;
    if (filter.edge && p.edgeId !== filter.edge && p.id !== filter.edge) return false;
    return true;
  });
  return matched.slice(-limit).map((e) => ({
    seq: e.seq,
    ts: e.ts,
    type: e.type,
    summary: summarize(e.type, e.payload as Record<string, unknown>),
    payload: e.payload as Record<string, unknown>,
  }));
}

function summarize(type: DEventType, p: Record<string, unknown>): string {
  switch (type) {
    case "TransitionTaken":
      return `${p.from ?? "-"} -> ${p.to} via ${p.edgeId}`;
    case "BlockedAttempt":
      return `blocked ${p.from ?? "-"} -> ${p.to}: ${p.reason}`;
    case "SoftViolation":
      return `soft violation on ${p.edgeId}: ${p.reason}`;
    case "RewardApplied":
      return `reward ${p.value} over ${(p.scopes as unknown[]).length} scopes`;
    case "NodeUpserted":
      return `node ${p.id} (${p.kind})`;
    case "EdgeUpserted":
      return `edge ${p.id}: ${p.src} -> ${p.dst} [${p.kind}]`;
    case "EdgeRemoved":
      return `edge removed ${p.id}`;
    case "ZoneDefined":
      return `zone ${p.name} (${(p.members as unknown[]).length} members)`;
    case "EnforcementChanged":
      return `enforcement ${p.scope} ${p.id} -> ${p.mode}`;
    case "CheckpointCreated":
      return `checkpoint ${p.name} @ ${p.seq}`;
    default:
      return type;
  }
}
