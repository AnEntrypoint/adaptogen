...
import type { Stat, Suggestion, EnforcementMode } from "./types.ts";
import type { Tunables } from "./config.ts";

/** ema reward decayed by recency: halves every `halfLife` seqs of staleness. */
export function decayedReward(stat: Stat | null, currentSeq: number, halfLife: number): number {
  if (!stat) return 0;
...
export function confidence(visits: number): number {
...
export function rank(moves: MoveStat[], cfg: Tunables, currentSeq: number, rand: () => number): Suggestion[] {
