...
import type { ValidationReport, Violation, IntegrityReport, NodeId } from "./types.ts";

export function verifyIntegrity(ds: DState): IntegrityReport {
  return ds.store.verifyIntegrity();
}

export function validate(ds: DState): ValidationReport {
...
export function repair(ds: DState): { fixed: Violation[]; quarantined: Violation[] } {
  const store = ds.store;
  const report = validate(ds);
  const fixed: Violation[] = [];
  const quarantined: Violation[] = [];
  ...
  for (const v of report.violations) {
    ...
    switch (v.kind) {
      ...
      case "DanglingZoneMember": {
        const [zone, node] = v.locus.split(":") as [string, NodeId];
