...
import { exportState, importState } from "./portability.ts";

export function checkpoint(ds: DState, name: string): Result<{ seq: number }> {
...
export function listCheckpoints(ds: DState): Array<{ name: string; seq: number }> {
  return ds.store.db.query<{ name: string; seq: number }>("SELECT name, seq FROM checkpoints ORDER BY seq").all();
}

export function rollback(ds: DState, name: string): Result<{ seq: number }> {
  const cp = ds.store.db.query<{ seq: number }>("SELECT seq FROM checkpoints WHERE name = ?").get(name);
...
export function branch(ds: DState, filename: string): Result<DState> {
 ...
 */
export function merge(mainDs: DState, branchDs: DState): Result<{ merged: number }> {
  const forkSeq = branchDs.store.getConfig<number>("__fork_seq");
  if (forkSeq == null) return fail("Conflict", "branch has no fork marker");
  const tail = branchDs.store
    .readEvents({ fromSeq: forkSeq + 1 })
    .filter((e) => !(e.type === "ConfigSet" && (e.payload as { key?: string }).key === "__fork_seq") && e.type !== "SnapshotTaken" && e.type !== "CheckpointCreated");
...
export function discard(branchDs: DState): void {
