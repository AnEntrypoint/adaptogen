...
export function render(ds: DState): string {
  const lines: string[] = [];
  ...
  if (rewards.length) {
    ...
    for (const r of rewards) lines.push(`  ${(r.payload as any).value} @ seq ${r.seq}`);
  }
...
export interface Metrics {
  nodes: { total: number; active: number; archived: number; deprecated: number };
  edges: { transition: number; dependency: number };
  zones: number;
  events: number;
  hotTransitions: Array<{ edge: string; visits: number }>;
  blocks: number;
...
export function metrics(ds: DState): Metrics {
...
export function toMermaid(ds: DState): string {
...
export function toDot(ds: DState): string {
