...
import { IdGen, isValidId } from "./ids.ts";
import {
  ok,
  fail,
  DStateError,
...
import type { Tunables } from "./config.ts";
import { compileGuard, evalGuard } from "./guard.ts";
import type { CompiledGuard } from "./guard.ts";
import { crossingInfo, boundaryMode, intraMode } from "./zone.ts";
import { decide } from "./enforce.ts";
import { rank } from "./intuition.ts";
import type { MoveStat } from "./intuition.ts";
import { dependencyCycle, readyFrontier, topoSort, reachable, ancestors, descendants } from "./graph.ts";
import type {
  DNode,
  DEdge,
  Zone,
...
export interface RememberInput {
  ...
  /** optimistic concurrency: reject if current version differs */
...
export interface RecallQuery {
  ...
  status?: NodeStatus;
  text?: string;
  /** query vector for similarity recall; ignored if no node has an embedding */
  embedding?: number[];
  limit?: number;
}
...
export interface RewardOptions {
  edgeId?: EdgeId;
  /** spread decayed credit over recent transitions (eligibility trace) */
  trace?: boolean;
  depth?: number;
  lambda?: number;
}
...
export interface DStateOptions extends StoreOptions {
  /** seed a starter self-model on a fresh store (default true) */
  seed?: boolean;
  /** acquire a single-writer lock file for on-disk stores (default true) */
  lock?: boolean;
}
...
export class DState {
  ...
  readonly ids: IdGen;
  private guardCache = new Map<string, Result<CompiledGuard>>();
  private tunablesCache: Tunables | null = null;
  private zoneMapCache: Map<string, Zone> | null = null;
  private opCount = 0;
  private lockPath: string | null = null;
  private now: () => number;
  private rand: () => number;

  private constructor(filename: string, opts: DStateOptions) {
  ...
  static open(filename = ":memory:", opts: DStateOptions = {}): DState {
  ...
  setTunable<K extends keyof Tunables>(key: K, value: Tunables[K]): Result<Tunables> {
  ...
  remember(input: RememberInput): Result<DNode> {
  ...
  setStatus(id: NodeId, status: NodeStatus): Result<DNode> {
  ...
  archive(id: NodeId): Result<DNode> {
    return this.setStatus(id, "archived");
  }
  deprecate(id: NodeId): Result<DNode> {
    return this.setStatus(id, "deprecated");
  }

  recall(query: RecallQuery = {}): DNode[] {
    ...
    if (query.text && this.store.ftsEnabled) {
      ...
      if (safe) {
        ...
        return rows.map((r) => this.store.getNode(r.id)!).filter(Boolean) as DNode[];
      }
    }
    ...
    params.push(limit);
    const rows = db.query<{ id: string }>(`SELECT id FROM nodes ${where} ORDER BY id LIMIT ?`).all(...params);
    return rows.map((r) => this.store.getNode(r.id)!).filter(Boolean) as DNode[];
  }

  // ---- edges -----------------------------------------------------------

  link(from: NodeId, to: NodeId, opts: LinkOptions = {}): Result<DEdge> {
    ...
    const kind: EdgeKind = opts.kind ?? "transition";
    if (opts.guard != null) {
  ...
  depend(node: NodeId, prereq: NodeId, opts: LinkOptions = {}): Result<DEdge> {
    return this.link(prereq, node, { ...opts, kind: "dependency" });
  }

  unlink(edgeId: EdgeId): Result<true> {
  ...
  setEnforcement(edgeId: EdgeId, mode: EnforcementMode): Result<DEdge> {
    if (!this.store.getEdge(edgeId)) return fail("NotFound", `edge ${edgeId} not found`);
  ...
  ready(done: Iterable<NodeId> = []): NodeId[] {
    return readyFrontier(this.store, new Set(done));
  }
  topo(): { order: NodeId[]; cyclic: boolean } {
    return topoSort(this.store);
  }
  reachable(from: NodeId, kind: EdgeKind = "transition"): NodeId[] {
    return [...reachable(this.store, from, kind)];
  }
  ancestors(of: NodeId, kind: EdgeKind = "dependency"): NodeId[] {
    return [...ancestors(this.store, of, kind)];
  }
  descendants(of: NodeId, kind: EdgeKind = "dependency"): NodeId[] {
  ...
  setCursor(nodes: NodeId[]): Result<NodeId[]> {
  ...
  legalMoves(vars: Record<string, unknown> = {}): MoveInfo[] {
    const out: MoveInfo[] = [];
  ...
  explainTransition(to: NodeId, vars: Record<string, unknown> = {}): Result<DecisionTrace> {
  ...
  transition(to: NodeId, vars: Record<string, unknown> = {}): Result<TransitionOutcome> {
    ...
    const drafts: Array<{ type: "SoftViolation" | "TransitionTaken"; payload: Record<string, unknown> }> = [];
  ...
  private decideTransition(edge: DEdge, from: NodeId | null, to: NodeId, vars: Record<string, unknown>): DecisionTrace {
    const cfg = this.getTunables();
    let guardPresent = edge.guard != null;
  ...
  private guardContext(edge: DEdge, from: NodeId | null, to: NodeId, vars: Record<string, unknown>): unknown {
  ...
  suggest(vars: Record<string, unknown> = {}): Suggestion[] {
    const moves = this.legalMoves(vars);
    const stats: MoveStat[] = moves.map((m) => ({
      edgeId: m.edgeId,
      to: m.to,
  ...
  reward(value: number, opts: RewardOptions = {}): Result<{ scopes: number }> {
    const alpha = this.getTunables().rewardAlpha;
    const scopes: Array<{ kind: string; id: string; weight: number }> = [];
    if (opts.edgeId) {
  ...
  recentTransitions(n: number): Array<{ edgeId: EdgeId; to: NodeId; from: NodeId | null }> {
    const evs = this.store.readEvents({ type: "TransitionTaken" });
    return evs
      .slice(-n)
      .reverse()
      .map((e) => ({ edgeId: (e.payload as any).edgeId, to: (e.payload as any).to, from: (e.payload as any).from ?? null }));
  }

  getStat(kind: "node" | "edge", id: string): Stat | null {
    return this.store.getStat(kind, id);
  }

  // ---- zones -----------------------------------------------------------

  defineZone(name: string, members: NodeId[], opts: { intra?: EnforcementMode; boundary?: EnforcementMode } = {}): Result<Zone> {
  ...
  addToZone(name: NodeId, node: NodeId): Result<Zone> {
  ...
  removeFromZone(name: string, node: NodeId): Result<Zone> {
    if (!this.store.getZone(name)) return fail("ZoneNotFound", `zone ${name} not found`);
   ...
   */
  deriveZone(seed: NodeId, predicate?: string): Result<{ members: NodeId[] }> {
    if (!this.store.getNode(seed)) return fail("NotFound", `seed ${seed} not found`);
    let guard: CompiledGuard | null = null;
    if (predicate) {
  ...
  compile(expr: string): Result<CompiledGuard> {
...
export type { Tunables } from "./config.ts";
